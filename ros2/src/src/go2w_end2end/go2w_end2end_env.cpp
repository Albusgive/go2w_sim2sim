#include "go2w_end2end_env.h"
#include "debug.hpp"

LowLevelCmdNode::LowLevelCmdNode(
    std::vector<std::pair<std::string, std::string>>
        &policy_paths_and_description)
    : ManagerBasedEnv(policy_paths_and_description), Node("end2end") {
...
void LowLevelCmdNode::initObsManager() {
  ...
  ray_caster_term->func = [this]() { return get_ray_caster_image2(); };
  ...
  auto action_obs_term2 =
      std::make_shared<ActionObsTerm>("action_obs_term", 1);
  ...
  pad->bindGamePadValues([this](GamePadValues map) {
    ...
    if (map.x) {
      ...
      if (policy_id == policy_description.size())
        policy_id = 0;
      Log("policy_description: " << policy_description[policy_id]);
    }
  });
  int is;
...
void LowLevelCmdNode::init_img_topic() {
  min_depth_ = 0.25;
  max_depth_ = 2.0;
  deep_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
      "/camera/depth/image_rect_raw", 1,
      std::bind(&LowLevelCmdNode::depth_callback, this, std::placeholders::_1));
  rgb_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
      "/camera/color/image_raw", 1,
      std::bind(&LowLevelCmdNode::rgb_callback, this, std::placeholders::_1));
  _obs_image = cv::Mat::zeros(cv::Size(20, 20), CV_32FC1);
}

void LowLevelCmdNode::rgb_callback(
...
void LowLevelCmdNode::depth_callback(
    const sensor_msgs::msg::Image::SharedPtr msg) {
...
cv::Mat LowLevelCmdNode::process_depth_image(cv::Mat &depth_image,
...
torch::Tensor LowLevelCmdNode::get_ray_caster_image2() {
  // obs
  /* Mat 2 vector */
