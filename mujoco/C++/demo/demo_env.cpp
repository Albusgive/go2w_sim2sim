...
MJ_ENV::MJ_ENV(std::string model_file,
               ...
               double max_FPS, int demo_mode)
    : ManagerBasedEnv(policy_paths_and_description), demo_mode(demo_mode) {
  ...
  int ray_caster_camera_size[2] = {20, 20};
  switch (demo_mode) {
  case 1: {
    ray_caster_camera_size[0] = 60;
    ray_caster_camera_size[1] = 30;
  } break;
  }

  ray_caster_base = RayCaster(m, d, "RayCaster_base", 0.2, {2.0, 1.0},
                              {0.01, 0.6}, RayCasterType::base);
  ray_caster_yaw = RayCaster(m, d, "RayCaster_yaw", 0.2, {2.0, 1.0},
  ...
  if (demo_mode == 1) {
    ...
    // ray_caster_camera.setNoise(
    //     ray_noise::RayNoise3(-0.01, 0.01, 0.005, 0.9, 16.5, 0.2, 0.8));
  }
  // body_track
  body_track("base_link", 0.02, {0.0, 1.0, 1.0, 0.5}, 50, 30);
  body_track("RR_wheel_link", 0.02, {1.0, 1.0, 0.0, 0.5}, 50, 30);
...
void MJ_ENV::step_unlock() {
  ray_update_setp++;
  if (ray_update_setp >= 3) {
    ray_update_setp = 0;
    switch (demo_mode) {
    case 0: {
      ray_caster_base.compute_distance();
      ray_caster_yaw.compute_distance();
      ray_caster_world.compute_distance();
      ray_caster_camera.compute_distance();
      ray_caster_lidar.compute_distance();

      ray_caster_base.get_image_data(ray_caster_base_img);
      ray_caster_yaw.get_image_data(ray_caster_yaw_img);
      ray_caster_world.get_image_data(ray_caster_world_img);
      ...
      ray_caster_camera.get_inv_image_data(ray_caster_camera_noise_inv_img,
                                           true);
    } break;
    case 1: {
      ray_caster_camera.compute_distance();
...
void MJ_ENV::draw() {
  ...
  case 0: {
    ...
    ray_caster_camera.draw_deep_ray(&scn, 2, 5, true, color4);
...
void MJ_ENV::draw_windows() {
  ...
  switch (demo_mode) {
  case 0: {
    drawGrayPixels(ray_caster_base_img, 0,
                   {ray_caster_base.h_ray_num, ray_caster_base.v_ray_num},
                   {150, 300});
    drawGrayPixels(ray_caster_yaw_img, 1,
                   {ray_caster_yaw.h_ray_num, ray_caster_yaw.v_ray_num},
                   {150, 300});
    drawGrayPixels(ray_caster_world_img, 2,
                   {ray_caster_world.h_ray_num, ray_caster_world.v_ray_num},
                   {300, 150});
    drawGrayPixels(ray_caster_camera_img, 3,
                   {ray_caster_camera.h_ray_num, ray_caster_camera.v_ray_num},
                   {300, 300});
    drawGrayPixels(ray_caster_lidar_img, 4,
...
void MJ_ENV::img_gradient() {
  cv::Mat blurred_gray(ray_caster_camera.v_ray_num, ray_caster_camera.h_ray_num,
                       CV_64FC1, ray_caster_camera.dist_ratio);

  cv::Mat img = blurred_gray.clone();
  img *= 255;
  // Scharr 算子 - 更精确的 Sobel 变种
  cv::Mat scharr_x, scharr_y;
  cv::Mat abs_scharr_x, abs_scharr_y, scharr_grad;

  cv::Scharr(img, scharr_x, CV_64FC1, 1, 0, 1, 0, cv::BORDER_DEFAULT);
  cv::convertScaleAbs(scharr_x, abs_scharr_x);

  cv::Scharr(img, scharr_y, CV_64FC1, 0, 1, 1, 0, cv::BORDER_DEFAULT);
  cv::convertScaleAbs(scharr_y, abs_scharr_y);

  cv::addWeighted(abs_scharr_x, 0.5, abs_scharr_y, 0.5, 0, scharr_grad);

  int ratio = 10;
  cv::resize(scharr_grad, scharr_grad,
             cv::Size(ray_caster_camera.h_ray_num * ratio,
                      ray_caster_camera.v_ray_num * ratio));
  cv::imshow("Scharr Gradient", scharr_grad);
